 ...
 * Helpers
 */
static inline int lock_trylock (lock_t *lock)
{
...
hashtable_entry hashtable_lock (value value)
{

    hash_t index;
    index = value->hash % HASHTABLE_SIZE;
    ...
    // Probe
    while (entry->value != EMPTY_SLOT && entry->value != DEAD_SLOT &&
           !uint128_equal (&value->id, &entry->value->id))
    {
...
void hashtable_unlock (hashtable_entry entry)
{
    assert (entry->value != EMPTY_SLOT && entry->value != DEAD_SLOT);

    lock_unlock (&entry->lock);
}

void hashtable_set_unlock (hashtable_entry entry, value value)
{
    assert (entry && lock_trylock (&entry->lock));
...
{
    assert (new != EMPTY_SLOT && new != DEAD_SLOT);
...
{
    assert (delete != EMPTY_SLOT && delete != DEAD_SLOT);
    ...
    while (entry->value != EMPTY_SLOT &&
           (entry->value == DEAD_SLOT
            || !uint128_equal (&delete->id, &entry->value->id)))
    {
        lock_unlock (&entry->lock);
        index = (index + 1) % HASHTABLE_SIZE;
        entry = hashtable + index;
...
{
    assert (find != EMPTY_SLOT && find != DEAD_SLOT);
    ...
    while (entry->value != EMPTY_SLOT &&
           (entry->value == DEAD_SLOT
            || !uint128_equal (&find->id, &entry->value->id)))
    {
...
{
    printf ("0x%016llx%016llx", (unsigned long long) id.h.msb,
            (unsigned long long) id.h.lsb);
}
...
int main ()
{
    ...
    for (uint64_t i = 0; i < HASHTABLE_SIZE; ++i)
        if (hashtable[i].value && hashtable[i].value != DEAD_SLOT)
        {
